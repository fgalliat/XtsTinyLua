...
static void math_rnd (void)
{
 ...
 lua_Object o1 = lua_getparam (1);
 if (!lua_isnumber(o1))
 { lua_error ("incorrect arguments to function `rnd'"); return; }
 d1 = lua_getnumber(o1);
 // beware c++ rand() is int
 lua_pushnumber ( (double)(rand() % 10000) / 10000.0 * d1 );
}
