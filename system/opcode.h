...
{
 char *s = (char*)calloc (strlen(l)+strlen(r)+2, sizeof(char));
 if (s == NULL)
 {
...
char *lua_strdup (char *l)
{
 char *s = (char*)calloc (strlen(l)+2, sizeof(char));
 if (s == NULL)
 {
  lua_error ("not enough memory");
  return NULL;
 ...
 while (1)
 {
  switch ((OpCode)*pc++)
  {
   ...
   case PUSHINDEXED:
    ...
    {
     Object *h = (Object*)lua_hashdefine (avalue(top-1), top);
     if (h == NULL) return 1;
     *(top-1) = *h;
    }
   break;
   ...
   case CREATEARRAY:
    ...
    avalue(top-1) = (Hash*)lua_createarray(lua_hashcreate(nvalue(top-1)));
...
//  if      (lua_isnumber(obj))    printf("%f",(float)lua_getnumber (obj));
// "%g" auto number format
  if      (lua_isnumber(obj))    printf("%g",(float)lua_getnumber (obj));
  else if (lua_isstring(obj))    printf(lua_getstring (obj));
  else if (lua_iscfunction(obj)) printf("cfunction: %p\n",lua_getcfunction (obj));
  else if (lua_isuserdata(obj))  printf("userdata: %p\n",lua_getuserdata (obj));
  else if (lua_istable(obj))     printf("table: %p\n",obj);
  else if (lua_isnil(obj))       printf("nil");
