...
void lua_error (char *s)
{
 ...
 else			printf("(EE) lua:%s\n", s); //fprintf (ext_stderr, "lua: %s\n", s);
}
...
void lua_reportbug (char *s)
{
 ...
 if (lua_debugline != 0)
 {
  ...
  if (nfuncstack > 0)
  {
   ...
   sprintf (strchr(msg,0), "\n\tactive stack\n");
   //sprintfOneStr(strchr(msg,0), "\n\tactive stack\n");


   for (i=nfuncstack-1; i>=0; i--)
    sprintf (strchr(msg,0), "\t-> function \"%s\" of file \"%s\"\n", 
//	sprintfTwoStrInStr(strchr(msg,0), "\t-> function \"%s\" of file \"%s\"\n", 
  ...
  {
   sprintf (strchr(msg,0),
   //sprintfIntStrInStr(strchr(msg,0),
