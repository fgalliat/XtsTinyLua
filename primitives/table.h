...
{
 int i, j, l=strlen(s);
 char *c = (char*)calloc (l, sizeof(char)); 	/* make a copy */
...
int lua_findconstant (char *s)
{
 ...
 {
  char *c = (char*)calloc(strlen(s)+2,sizeof(char));
  c++;		/* create mark space */
...
void lua_nextvar (void)
{
 ...
 else
 {
  for (index=0; index<lua_ntable; index++)
   if (streq((char*)s_name(index),(char*)svalue(o))) break;
  if (index == lua_ntable) 
  {
